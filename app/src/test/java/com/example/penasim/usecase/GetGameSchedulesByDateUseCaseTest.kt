...
class GetGameSchedulesByDateUseCaseTest {
    ...
    @Test
    fun execute_returnsSchedules_forDate() = runTest {
        ...
        val useCase = GetGameSchedulesByDateUseCase(
            FakeGameFixtureRepository(fixtures),
            FakeTeamRepository(teams)
        )
        val schedules = useCase.execute(date)
    ...
    fun execute_throws_whenTeamMissing() = runTest {
        ...
        assertFailsWith<IllegalArgumentException> { useCase.execute(date) }
    }
}
