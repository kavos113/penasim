...
data class CommandUiState(
  ...
  fun getOrderFielderAppointments(orderType: OrderType): List<FielderAppointment> =
    fielderAppointments.filter { it.orderType == orderType }.filter { it.position.isStarting() }
      .sortedBy { it.number }

  fun getBenchFielderAppointments(orderType: OrderType): List<FielderAppointment> =
    fielderAppointments.filter { it.orderType == orderType }
      .filter { it.position == Position.BENCH }.sortedBy { it.number }
