...
class MainMembersUseCase @Inject constructor(
  ...
  suspend fun getByPlayerId(playerId: Int): MainMember? =
    repository.getMainMemberByPlayerId(playerId)
  ...
  suspend fun updateOnlyDiff(members: List<MainMember>) {
    ...
    require(members.map { it.teamId }
      .distinct().size == 1) { "Members must belong to the same team" }
    require(members.map { it.playerId }
      .distinct().size == members.size) { "Duplicate player ID in members" }
