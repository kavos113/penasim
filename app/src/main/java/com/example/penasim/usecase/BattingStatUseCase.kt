...
class BattingStatUseCase @Inject constructor(
    ...
    suspend fun getByFixtureId(fixtureId: Int): List<BattingStat> = repository.getByFixtureId(fixtureId)
    suspend fun getByFixtureIds(fixtureIds: List<Int>): List<BattingStat> = repository.getByFixtureIds(fixtureIds)
    suspend fun getByPlayerId(playerId: Int): List<BattingStat> = repository.getByPlayerId(playerId)
    suspend fun getByPlayerIds(playerIds: List<Int>): List<BattingStat> = repository.getByPlayerIds(playerIds)
